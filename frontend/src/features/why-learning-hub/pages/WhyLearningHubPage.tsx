...
export default function WhyLearningHubPage() {
      ...
      <main className="lp-home-main">
        <section className="lp-why-page-intro">
          <div className="lp-why-page-breadcrumb-wrap">
            ...
            <span className="lp-why-page-breadcrumb-current">Why Learning Hub</span>
          </div>
          <div className="lp-why-page-intro-inner">
            <h1>WHERE CONCEPTS BECOME STRENGTH</h1>
            <p>
              AT <strong>LEARNING HUB</strong>, WE DON&apos;T JUST PREPARE STUDENTS FOR EXAMS-WE BUILD THE THINKING ABILITY THAT DRIVES SUCCESS IN ACADEMICS AND BEYOND.
            </p>
            <hr className="lp-why-divider" />
            <img src={whyLearningHubImage} alt="Learning Hub students" className="lp-why-banner-image" />
            <h2 className="lp-why-features-title">Why Learning Hub Stands Apart</h2>
            <div className="lp-why-features-grid lp-why-features-grid--desktop">
              {WHY_FEATURES.map((feature, index) => renderFeatureCard(feature, `desktop-${index}`))}
            </div>
            <div className="lp-why-features-mobile">
              <div
                className="lp-why-features-mobile-scroll"
                onTouchStart={(e) => setFeatureTouchStartX(e.touches[0].clientX)}
                onTouchEnd={(e) => {
                  if (featureTouchStartX === null) return;
                  const deltaX = e.changedTouches[0].clientX - featureTouchStartX;
                  if (deltaX < -40) showNextFeatureSlide();
                  if (deltaX > 40) showPrevFeatureSlide();
                  setFeatureTouchStartX(null);
                }}
                onWheel={(e) => {
                  ...
                  if (e.deltaX > 0) showPrevFeatureSlide();
                  if (e.deltaX < 0) showNextFeatureSlide();
                }}
              >
                <div className="lp-why-features-mobile-track">
                  {mobileScrollableFeatureGroups.map((group, groupIndex) => (
                    <div
                      className={`lp-why-features-mobile-slide ${groupIndex === activeFeatureSlide ? "lp-why-features-mobile-slide--active" : ""}`}
                      key={`mobile-slide-${groupIndex}`}
                    >
                      {group.map((feature, featureIndex) =>
                        renderFeatureCard(feature, `mobile-slide-${groupIndex}-${featureIndex}`)
                      )}
                    </div>
                  ))}
                </div>
              </div>
              <div className="lp-home-dots" aria-label="feature carousel indicators">
                {mobileScrollableFeatureGroups.map((_, idx) => (
                  <button
                    key={`feature-dot-${idx}`}
                    type="button"
                    className={`lp-home-dot ${activeFeatureSlide === idx ? "lp-home-dot--active" : ""}`}
            ...
            <div className="lp-why-content-cards">
              <article className="lp-why-content-card">
                <h2>More Than Teaching — A Learning Transformation</h2>
                <p>
                  At <strong>Learning Hub</strong>, teaching is not just about completing the syllabus. It&apos;s about
                  helping students:
                </p>
                <ul className="lp-why-list">
                  <li>Think independently</li>
                  <li>Solve confidently</li>
                  <li>Learn consistently</li>
                </ul>
                <p>
                  Through structured guidance and practical explanation, students gradually transform into
                  self-reliant learners.
                </p>
              </article>

              <article className="lp-why-content-card">
                <h2>Learn From Experience, Not Just Theory</h2>
                <p>
                  With strong engagement through our YouTube channel, we understand real student challenges—where
                  they struggle, what they misunderstand, and how they improve.
                </p>
                <p>This real-world insight shapes every lesson we teach.</p>
              </article>
            </div>
